...
class MAMLIL(BatchMAMLPolopt):
    ...
    def init_opt(self):
        ...
        #     paths_range = range(int(self.batch_size / self.meta_batch_size))
        #     # print("debug, paths_range\n", paths_range)
        #     # temp1 = tf.reduce_sum(tf.reshape(old_logli_sym[0][i], [ -1,1]), 1)
        #     temp1 = tf.reshape(old_logli_sym[0][i], [ -1,1])
        ...
        #     # temp2 = tf.reduce_mean(tf.reshape(old_logli_sym[0][i] * old_adv[0][i], [-1,1]), 1)
        #     temp2 = tf.reshape(old_logli_sym[0][i] * old_adv[0][i], [-1,1])
